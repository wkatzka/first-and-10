...
export async function getRosterStrategy() {
  return request('/roster/strategy');
}

export async function fillToRatio(side, targetRatio) {
  return request('/roster/fill-to-ratio', {
    method: 'POST',
    body: JSON.stringify({ side, targetRatio }),
  });
}

export async function autoFillRoster(strategy = 'balanced', defenseStrategy = 'base_defense') {
  return request('/roster/auto-fill', {
    method: 'POST',
    body: JSON.stringify({ strategy, defenseStrategy }),
  });
}

export async function getRosterPresets(side = 'offense') {
  return request(`/roster/presets?side=${side}`);
}

export async function getUserPresets(userId, side = 'offense') {
  return request(`/users/${userId}/presets?side=${side}`);
}

export async function applyRosterPreset(side, slots) {
  return request('/roster/apply-preset', {
    method: 'POST',
    body: JSON.stringify({ side, slots }),
  });
}

export async function getRosterLockStatus() {
  return request('/roster/lock-status');
}
...
export async function getH2HRecords() {
  return request('/leaderboard/h2h');
}
