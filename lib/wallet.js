...
export function useWallet() {
  ...
  const switchNetwork = useCallback(async () => {
    // For Web3Auth users, they're already configured for Base Sepolia
    if (ctx?.walletType === 'web3auth') {
      alert('Your wallet is already configured for Base Sepolia. If you see wrong network, try disconnecting and reconnecting.');
      return;
    }
