/**
 * useWallet – Unified wallet hook for Shop.
 * Supports both Farcaster Mini App (wagmi) and traditional Web3Auth/MetaMask.
 */
import { useCallback } from 'react';
import { BASE_SEPOLIA_CHAIN_ID } from './contracts';
import { useWalletContext } from './Web3AuthContext';
import { useFarcasterWallet } from './FarcasterWalletContext';

export function useWallet() {
  // Try Farcaster context first (will be null if not in Farcaster)
  const farcasterCtx = useFarcasterWallet();
  
  // Fall back to Web3Auth context
  const web3authCtx = useWalletContext();
  
  // Use Farcaster if available and we're in a Farcaster Mini App
  const ctx = farcasterCtx?.isFarcaster ? farcasterCtx : web3authCtx;
